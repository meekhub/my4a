CREATE OR REPLACE PROCEDURE ALLDM.P_BWT_CNTRT_MGMT_UNIT_M(V_MONTH   VARCHAR2,
...
BEGIN
  ...
  IF V_COUNT1 >= 0 THEN
    ...
    EXECUTE IMMEDIATE 'TRUNCATE TABLE MID_CONTRACT_INFO';
    --合同信息
    insert into MID_CONTRACT_INFO 
    select a.CONTRACTID,
       b.STATUS,
       b.SIGNCODE,
       b.CONTRACTCODE,
 ...
 where a.CONTRACTID = b.ID
   ...
   and b.contractname not like '%解除%';
   commit;
      ...
      SELECT V_MONTH,
             ...
             CASE
               ...
               WHEN A.HUAXIAO_NO = '813060011010000' THEN
                '市辖区'
               WHEN A.HUAXIAO_NO IN ('813050340720000','813050340740000','813050340710000') THEN 
                 '清河县'
               ELSE
                B.CITY_DESC
             END,
       ...
       WHERE A.CITY_NO = B.CITY_NO
         AND A.HUAXIAO_TYPE IN
            -- ('01', '02', '03', '04', '05', '06', '07', '08', '09')  经企发部王婕要求暂时去掉后端划小
            ('01', '02', '03', '04', '05', '06', '07', '08')
         AND A.HUAXIAO_NO = C.SMALLCODE
         and a.if_valid='1'
         and c.status<>6
         and c.CONTRACTCODE is not null;
    COMMIT;
