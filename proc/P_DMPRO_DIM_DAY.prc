...
BEGIN
  ...
  IF V_COUNT >= 0 THEN
    ...
    COMMIT;
    --更新区县名称
      UPDATE DIM.DIM_CITY_NO_TRANS T
         SET T.CITY_DESC =
             (SELECT T2.CITY_DESC
                FROM DIM.DIM_CITY_NO T2
               WHERE T2.CITY_NO = T.CITY_NO),
    ...
    --20180227 柴占飞根据马建辉要求：由于决策专家系统需要展示，不在剔除承德的这4个区县
    ...
    END IF;
      ...
      INSERT INTO ALLDMCODE.DMCODE_XIAOQU_INFO
        SELECT TRIM(B.AREA_NO), --
          ...
          FROM (SELECT T.*,
                       ROW_NUMBER() OVER(PARTITION BY XIAOQU_NO ORDER BY BEGIN_DATE DESC, SHENTOU_RATE DESC) RN
                  FROM STAGE.XIAOQU_INFO@HBODS T
                 WHERE T.AREA_NO IS NOT NULL) A,
      ...
      --======================== 柴占飞 20180608 增加用户状态码表定时更新 =================
       INSERT INTO DIM.DIM_USER_STATUS
         SELECT A.KIND USER_STATUS,
                A.NAME USER_STATUS_DESC,
                '' STOP_TYPE,
                A.KIND USER_STATUS_NEUSOFT,
                A.NAME USER_STATUS_DESC_NEUSOFT
         
           FROM (SELECT * FROM CRM_DSG.BB_SERVING_STATUS_T@HBODS T) A,
                
                (SELECT * FROM DIM.DIM_USER_STATUS) B
         
          WHERE A.KIND = B.USER_STATUS_NEUSOFT(+)
            AND B.USER_STATUS_NEUSOFT IS NULL;
       COMMIT;
...
/*    INSERT INTO DIM.DIM_XIAOQU_HUAXIAO
      ...
      INSERT INTO DIM.DIM_HUAXIAO_INFO
        SELECT AREA_NO,
               ...
               HUAXIAO_TYPE_BIG,
               HUAXIAO_TYPE_NAME_BIG,
               CASE WHEN IF_VALID='1' THEN MANAGER_LOGINID ELSE '' END,
               CASE WHEN IF_VALID='1' THEN MANAGER_LOGINNAME ELSE '' END,
               CASE WHEN IF_VALID='1' THEN MANAGER_TELEPHONE ELSE '' END
          FROM STAGE.ITF_OTHER_HUAXIAO_INFO@HBODS T
         WHERE T.DAY_ID = V_DATE
           AND T.HUAXIAO_NAME <> '乙类项目'
           AND T.HUAXIAO_TYPE IS NOT NULL;
      COMMIT;
   ...
   --========================= 备份表 ==========================================
    ...
    --删除在划小基础表没有而关系表存在的无效记录
    DELETE FROM DIM.DIM_CHANNEL_HUAXIAO
 WHERE HUAXIAO_NO IN
       (SELECT DISTINCT A.HUAXIAO_NO
          FROM DIM.DIM_CHANNEL_HUAXIAO A, DIM.DIM_HUAXIAO_INFO B
         WHERE A.HUAXIAO_NO = B.HUAXIAO_NO(+)
           AND B.HUAXIAO_NO IS NULL);
           COMMIT;

    DELETE FROM DIM.DIM_XIAOQU_HUAXIAO
 WHERE HUAXIAO_NO IN
       (SELECT DISTINCT A.HUAXIAO_NO
          FROM DIM.DIM_XIAOQU_HUAXIAO A, DIM.DIM_HUAXIAO_INFO B
         WHERE A.HUAXIAO_NO = B.HUAXIAO_NO(+)
           AND B.HUAXIAO_NO IS NULL);
           COMMIT;

    DELETE FROM DIM.DIM_ZQ_CHANNEL_HUAXIAO
 WHERE HUAXIAO_NO IN
       (SELECT DISTINCT A.HUAXIAO_NO
          FROM DIM.DIM_ZQ_CHANNEL_HUAXIAO A, DIM.DIM_ZQ_HUAXIAO_INFO B
         WHERE A.HUAXIAO_NO = B.HUAXIAO_NO(+)
           AND B.HUAXIAO_NO IS NULL);
           COMMIT;
 ...
 WHERE HUAXIAO_NO IN
       (SELECT DISTINCT A.HUAXIAO_NO
          FROM DIM.DIM_ZQ_XIAOQU_HUAXIAO A, DIM.DIM_ZQ_HUAXIAO_INFO B
         WHERE A.HUAXIAO_NO = B.HUAXIAO_NO(+)
           AND B.HUAXIAO_NO IS NULL);
           COMMIT;

    DELETE FROM DIM.DIM_ZQ_GROUP_HUAXIAO
 WHERE HUAXIAO_NO IN
       (SELECT DISTINCT A.HUAXIAO_NO
          FROM DIM.DIM_ZQ_GROUP_HUAXIAO A, DIM.DIM_ZQ_HUAXIAO_INFO B
         WHERE A.HUAXIAO_NO = B.HUAXIAO_NO(+)
           AND B.HUAXIAO_NO IS NULL);
           COMMIT;          
  END IF;
  ...
  ---=================================20180629 柴占飞增加： 更新标准地址码表，政企划小用到  ============================
  
  FOR C1 IN V_AREA LOOP
    
      ----------------------- 更新标准地址  -----------------------------
      SELECT NVL(MAX(TO_NUMBER(SUBSTR(T.STDADDR_NO, 13))), 0)
        INTO V_STDADDR_NO
        FROM DIM.DIM_STANDARD_ADDRESS_NO T
       WHERE T.AREA_NO = C1.AREA_NO;
    
      EXECUTE IMMEDIATE 'TRUNCATE TABLE DMPRO.MID_STD_ADDRESS';
    
      INSERT INTO DMPRO.MID_STD_ADDRESS
        SELECT GRADE_0,
               GRADE_1,
               GRADE_2,
               GRADE_3,
               GRADE_4,
               '' INNET_METHDO,
               'STDADDR_' || C1.AREA_NO || '_' ||
               TO_CHAR(V_STDADDR_NO + ROWNUM) STDADDR_NO,
               STDADDR_NAME,
               
               GRADE_5
        
          FROM (SELECT DISTINCT GRADE_0,
                                GRADE_1,
                                DECODE(T.GRADE_2,
                                       '唐海县',
                                       '曹妃甸区',
                                ...
                                GRADE_0 || '->' || GRADE_1 || '->' ||
                                T.GRADE_2 || '->' || T.GRADE_3 || '->' ||
                                T.GRADE_4 || '->' || T.GRADE_5 STDADDR_NAME
                
                 FROM(SELECT REPLACE(T.GRADE_0, '？', '') GRADE_0,
                             REPLACE(T.GRADE_1, '？', '') GRADE_1,
                             REPLACE(T.GRADE_2, '？', '') GRADE_2,
                             REPLACE(T.GRADE_3, '？', '') GRADE_3,
                             REPLACE(T.GRADE_4, '？', '') GRADE_4,
                             REPLACE(T.GRADE_5, '？', '') GRADE_5
                  FROM DW.DW_V_USER_ADSL_EIGHT_D T
                 WHERE ACCT_MONTH = SUBSTR(V_DATE, 1, 6)
                   AND DAY_ID = SUBSTR(V_DATE, 7, 2)
                   AND T.AREA_NO = C1.AREA_NO
                   AND T.GRADE_5 IS NOT NULL) T
      ...
      --取区县ID
      EXECUTE IMMEDIATE 'TRUNCATE TABLE DMPRO.MID_STD_ADDR_CITY_NO';
      INSERT INTO DMPRO.MID_STD_ADDR_CITY_NO
        SELECT DISTINCT T.AREA_NO, T.CITY_NO, T.GRADE_2_DESC
          FROM DIM.DIM_STANDARD_ADDRESS_NO T
         WHERE T.AREA_NO = C1.AREA_NO;
      COMMIT;
    
      --正式数据
      INSERT INTO DIM.DIM_STANDARD_ADDRESS_NO
        SELECT T2.AREA_NO AREA_NO,
               T2.CITY_NO CITY_NO,
               '6' STDADDR_TYPE,
               T.STDADDR_NO,
               T.STDADDR_NAME,
          ...
          FROM DMPRO.MID_STD_ADDRESS T, DMPRO.MID_STD_ADDR_CITY_NO T2
         WHERE T.GRADE_2_DESC = T2.CITY_DESC;
      COMMIT;
