CREATE OR REPLACE PROCEDURE P_DM_BS_STATION_RRU(V_ACCT_MONTH VARCHAR2,
  ...
  IF V_COUNT >= 1 THEN
    ...
    EXECUTE IMMEDIATE 'TRUNCATE TABLE MID_STATION_DEL_INFO_R';
     INSERT INTO MID_STATION_DEL_INFO_R
       ...
       UNION
       SELECT DISTINCT A.XIAOQU_NO
         FROM ODS.O_RES_BS_STATION_RRU@HBODS A, ODS.O_RES_BS_STATION_XIAOQU@HBODS B
        WHERE A.XIAOQU_NO = B.XIAOQU_NO
          AND (B.FG_TYPE = '室分覆盖' OR B.PINDUAN IN ('3','5'))
          AND A.ACCT_MONTH = V_ACCT_MONTH
          AND B.ACCT_MONTH = V_ACCT_MONTH
       UNION
