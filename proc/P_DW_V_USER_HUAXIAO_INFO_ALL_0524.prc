CREATE OR REPLACE PROCEDURE DW.P_DW_V_USER_HUAXIAO_INFO_ALL(V_ACCT_MONTH VARCHAR2,
...
BEGIN
  ...
  INTO DW_V_USER_HUAXIAO_INFO_ALL
    SELECT /*+ORDERED*/
     ...
     CASE
       WHEN J3.CHANNEL_NO IS NOT NULL THEN
        '1'
       ELSE
        '0'
     END IS_HUAXIAO_07, --是否校园承包-政企类
     CASE
       WHEN J4.HUAXIAO_NO IS NOT NULL THEN
        '1'
       ELSE
        '0'
     ...
     A.IS_XINXING --是否新兴业务（不含物联网）
      FROM (SELECT A.AREA_NO,
           ...
           (SELECT * FROM MID_ZQ_V_USER_HUAXIAO_M_PLUS) F, --融合宽带用户
           ...
           MID_ZQ_HUAXIAO_YW J2, --营维
           (SELECT *
              FROM DIM.DIM_CHANNEL_HUAXIAO A
             WHERE A.HUAXIAO_TYPE = '07') J3, --校园
