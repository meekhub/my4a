...
BEGIN
  ...
  V_LAST_MONTH := TO_CHAR(ADD_MONTHS(TO_DATE(V_MONTH, 'YYYYMM'), -1),
                          'YYYYMM');
  V_LAST_5DAYS := TO_CHAR(TO_DATE(V_DATE, 'YYYYMMDD') - 5, 'YYYYMMDD');
  ...
  SELECT COUNT(1)
   ...
   WHERE ACCT_MONTH = V_DATE
     AND PROCNAME IN
         ('P_DW_V_USER_BASE_INFO_DAY', 'P_DW_V_USER_SCHOOL_HX_USER_D')
     AND RESULT = 'SUCCESS';
  ...
  IF V_CNT = 2 THEN
    ...
    FOR C1 IN V_AREA LOOP
      ...
      EXECUTE IMMEDIATE 'ALTER TABLE DW_V_USER_HUAXIAO_INFO_D TRUNCATE SUBPARTITION PART' ||
                        SUBSTR(V_LAST_5DAYS, 7, 2) || '_SUBPART_' ||
                        C1.AREA_NO;
    
      INSERT INTO DW_V_USER_HUAXIAO_INFO_D
        SELECT /*+ORDERED*/
         V_MONTH,
         V_DAY,
         A.AREA_NO,
        ...
        --NVL(G3.IOT_FEE, 0)
          FROM (SELECT A.AREA_NO,
               ...
               (SELECT USER_NO, XIAOQU_NO, XIAOQU_NAME
                 ...
                 WHERE AREA_NO = C1.AREA_NO) G, --上月沉淀的基础表
               (SELECT *
                  FROM DIM.DIM_XIAOQU_HUAXIAO
    ...
    --更新日志
    V_RETCODE := 'SUCCESS';
    P_UPDATE_LOG(V_DATE, V_PKG, V_PROCNAME, '结束', 'SUCCESS', SYSDATE);
  ELSE
    V_RETCODE := 'WAIT';
