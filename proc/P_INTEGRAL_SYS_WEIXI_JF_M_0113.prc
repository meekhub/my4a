CREATE OR REPLACE PROCEDURE P_INTEGRAL_SYS_WEIXI_JF_M(V_MONTH   VARCHAR2,
  ...
  /*****************************************************************
  *名称 --%NAME: P_INTEGRAL_SYS_WEIXI_JF_M
  *功能描述 --%COMMENT:
  *执行周期 --%PERIOD: 月
...
BEGIN
  ...
  SELECT COUNT(1)
   ...
   WHERE ACCT_MONTH = V_MONTH
     AND RESULT = 'SUCCESS'
     AND PROCNAME IN
         ('P_DW_V_USER_BASE_INFO_USER', 'P_DW_V_USER_PAY_LOG_MONTH','P_DW_V_USER_DEVICE_RENT');
  COMMIT;
    ...
    FOR C1 IN V_AREA LOOP
          ...
          FROM MID_JF_WEIXI_ZZ_04 A,
               (SELECT ACCT_MONTH,
                 ...
                 WHERE ACCT_MONTH = V_MONTH
                   --AND AREA_NO = C1.AREA_NO
                   AND TELE_TYPE = '2'
                   AND (IS_ACCT='1' OR IS_ACCT_OCS='1')) B
        ...
        SELECT A.*, B.LOW_VALUE
          FROM (SELECT ACCT_MONTH,
                 ...
                 WHERE ACCT_MONTH = V_MONTH
                   ...
                   --AND IS_VALID = '1'
                   AND (IS_ACCT='1' OR IS_ACCT_OCS='1')
                   AND TELE_TYPE = '2'
      ...
      INSERT INTO MID_INTEGRAL_SYS_WEIXI_JF_3
        SELECT ACCT_MONTH,
               ...
               A.USER_DINNER,
               CASE
                 WHEN (C.LOW_VALUE = 59 OR C.LOW_VALUE = 129) AND
                      A.FLAG = '1' THEN
                  20
                 WHEN C.LOW_VALUE = 99 AND A.FLAG = '1' THEN
                  40
                 WHEN (C.LOW_VALUE = 99 OR C.LOW_VALUE = 129) AND
                      A.FLAG = '0' THEN
                  20
                 WHEN C.LOW_VALUE = 199 AND A.FLAG = '0' THEN
                  50
               END LOW_VALUE
          FROM (SELECT A.*,
                       CASE
                         WHEN A.USER_DINNER_DESC LIKE '%不限量%' AND
                              A.LOW_VALUE = 109 THEN
                          '1'
                         ELSE
                          '0'
                       END FLAG
                  FROM MID_INTEGRAL_SYS_WEIXI_JF_1 A
                 WHERE A.USER_DINNER_DESC LIKE '%不限量%'
                   AND (A.LOW_VALUE = 109 OR A.LOW_VALUE = 199)) A,
               (SELECT USER_NO, USER_DINNER
                  FROM DW.DW_V_USER_BASE_INFO_USER T
                 WHERE ACCT_MONTH = V_LAST_MONTH
                   AND AREA_NO = C1.AREA_NO
         ...
         WHERE A.USER_NO = B.USER_NO
           AND B.USER_DINNER = C.USER_DINNER
           AND C.USER_DINNER_DESC LIKE '%乐享%';
      COMMIT;
          ...
          LEFT JOIN (SELECT *
                       FROM (SELECT ACCEPT_PERSON,
                              ...
                              WHERE ACCT_MONTH = V_LAST_MONTH
                                ...
                                AND DEALER_ID<>'-1'
                                AND ACCEPT='1'
                                AND ACCEPT_DATE >= TO_DATE(V_LAST_MONTH, 'YYYYMM')
                                AND ACCEPT_DATE <
                                    ADD_MONTHS(TO_DATE(V_LAST_MONTH, 'YYYYMM'), 1)
                                AND APPLY_EVENT = '302')
                      WHERE RN = 1) B
    ...
    INSERT INTO INTEGRAL_SYS_WEIXI_JF_M
      SELECT V_MONTH,
             ...
             CASE
               ...
               WHEN TELE_TYPE = '3' THEN
                '移动用户续约积分'
               WHEN TELE_TYPE = '4' THEN
                '移动缴赠业务积分'
               WHEN TELE_TYPE = '5' THEN
                '维系奖励积分'
             END TELE_TYPE,
             C.AGENT_ID,
             C.AGENT_NAME,
      ...
      --AND B.VALID_STATUS = '1'
       GROUP BY B.AREA_NO,
                ...
                END;
    COMMIT;
    delete from INTEGRAL_SYS_WEIXI_JF_M where acct_month=v_month and low_value=0;
    commit;
    --更新日志
    V_RETCODE := 'SUCCESS';
