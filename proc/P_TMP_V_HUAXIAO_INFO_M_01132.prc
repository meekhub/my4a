CREATE OR REPLACE PROCEDURE P_TMP_V_HUAXIAO_INFO_M(V_ACCT_MONTH VARCHAR2,
...
BEGIN
  ...
  IF V_COUNT = 1 THEN
    ...
    --删除当月数据
    DELETE FROM ALLDM.TMP_V_HUAXIAO_INFO_M WHERE ACCT_MONTH = V_ACCT_MONTH;
    COMMIT;
    --正式数据
    FOR C1 IN V_AREA LOOP
      DELETE FROM ALLDM.TMP_V_HUAXIAO_INFO_M
       WHERE ACCT_MONTH = V_ACCT_MONTH
         AND AREA_NO = C1.AREA_NO;
      COMMIT;
      ---自营厅
      INSERT /*+APPEND*/
      INTO ALLDM.TMP_V_HUAXIAO_INFO_M
        SELECT V_ACCT_MONTH,
               C1.AREA_NO,
               CITY_NO,
               HUAXIAO_TYPE,--划小类型
          ...
          FROM (SELECT T.CITY_NO,
                       ...
                       SUM(ONNET_CDMA_NUM) JZ_CDMA_NUM,
                ...
                SELECT CITY_NO,
                       ...
                       0 SCHOOL_GIVE_SORE,
                       0 SCHOOL_SUM_SORE,
                       ONNET_CDMA_NUM * (-1) JZ_CDMA_NUM,
                       ONNET_ADSL_NUM * (-1) JZ_ADSL_NUM,
                       ONNET_IPTV_NUM * (-1) JZ_IPTV_NUM
                  FROM ALLDM.TMP_V_HUAXIAO_INFO_M 
