CREATE OR REPLACE PROCEDURE P_TMP_V_USER_HUAXIAO_0314(V_ACCT_MONTH VARCHAR2,
  ...
  IF V_CNT = 1 THEN
    ...
    SELECT USER_ID, SUM(ARREAR_FEE) ARREAR_FEE
      FROM (SELECT CITY_CODE, USER_ID, NVL(ARREAR_FEE, 0)
              FROM ACCT_DSG.BRPT_THREE_ARREAR_USER_1_T@HBODS A
             WHERE STAT_MONTH = V_ACCT_MONTH
               AND FEE_DATE > = '200810'
               AND CITY_CODE = C1.AREA_NO
            UNION ALL
            SELECT CITY_CODE, USER_ID, NVL(ARREAR_FEE, 0) * (-1)
              FROM ACCT_DSG.BRPT_THREE_ARREAR_USER_1_T@HBODS A
             WHERE STAT_MONTH = V_LAST_MONTH
               AND CITY_CODE = C1.AREA_NO
               AND FEE_DATE > = '200810')
     GROUP BY USER_ID;
    COMMIT;
    ...
    COMMIT;
      ...
      INTO DW_V_USER_HUAXIAO_INFO_M
        SELECT /*+ORDERED*/
         ...
         CASE
           WHEN H3.USER_DINNER IS NULL AND
                A.TELE_TYPE_NAME IN ('移动', '宽带', '电视', '固话') AND
         ...
         H2.XMFC
          FROM (SELECT A.AREA_NO,
               ...
               (SELECT T.USER_NO, T.IOT_FEE
                ...
                DIM.DIM_ZS_SCHOOL_DINNER H3--校园专属套餐
         WHERE A.USER_NO = B.USER_NO(+)
           AND B.STDADDR_NAME = C.STDADDR_NAME(+)
           ...
           AND A.USER_DINNER = H3.USER_DINNER(+)
