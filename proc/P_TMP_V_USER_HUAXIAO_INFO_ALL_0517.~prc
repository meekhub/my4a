CREATE OR REPLACE PROCEDURE P_TMP_V_USER_HUAXIAO_INFO_ALL(V_ACCT_MONTH VARCHAR2,
    ...
    SELECT /*+ORDERED*/
     ...
     CASE
       WHEN J1.USER_NO IS NOT NULL THEN
        '1'  --存量
       WHEN A.IS_KD_BUNDLE = '0' AND A.TELE_TYPE = '2' AND G.XIAOQU_NO IS NOT NULL AND G4.HUAXIAO_TYPE = '05' THEN
        '1' --对于融合解绑的用户，解绑后还是按照融合时固网装机地址
       when (A.IS_KD_BUNDLE = '0' AND A.TELE_TYPE = '2' AND D5.HUAXIAO_TYPE = '05' AND G.XIAOQU_NO IS NULL) or j3.huaxiao_type='05' THEN
        '1' --单C
       when A.IS_KD_BUNDLE = '0' AND A.TELE_TYPE <> '2'  AND E3.HUAXIAO_TYPE = '05' THEN
        '1'  --单固
       when F1.HUAXIAO_TYPE = '05' THEN
        '1' --融合
       ELSE
        '0'
     ...
     A.IS_XINXING --是否新兴业务（不含物联网）
      FROM (SELECT A.AREA_NO,
           ...
           (SELECT * FROM DIM.DIM_ZQ_CHANNEL_HUAXIAO WHERE HUAXIAO_TYPE = '05') D5, --农村支局
           ...
           MID_ZQ_HUAXIAO_DEVELOP J3 --营维【增量】
     WHERE A.USER_NO = B.USER_NO(+)
       AND B.STDADDR_NAME = C.STDADDR_NAME(+)
