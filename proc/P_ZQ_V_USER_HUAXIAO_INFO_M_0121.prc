CREATE OR REPLACE PROCEDURE P_ZQ_V_USER_HUAXIAO_INFO_M(V_ACCT_MONTH VARCHAR2,
...
BEGIN
  ...
  V_PKG      := '政企用户划小信息月表';
  V_PROCNAME := 'P_ZQ_V_USER_HUAXIAO_INFO_M';
  ...
  IF V_CNT = 1 THEN
    ...
    FOR C1 IN V_AREA LOOP
      ...
      --取本月固网融合用户
      EXECUTE IMMEDIATE 'TRUNCATE TABLE ZQ_DW_V_USER_HUAXIAO_M';
      INSERT INTO ZQ_DW_V_USER_HUAXIAO_M
        SELECT /*+ORDERED*/
         ...
         E.HUAXIAO_TYPE_NAME
          ...
          FROM (SELECT AREA_NO,
               ...
               (SELECT *
                    FROM (SELECT X.*,
                                 ROW_NUMBER() OVER(PARTITION BY XIAOQU_NO ORDER BY HUAXIAO_NO ASC) RN
                            FROM (SELECT *
                                    FROM DIM.DIM_XIAOQU_HUAXIAO
                                  UNION ALL
                                  SELECT * FROM DIM.DIM_ZQ_XIAOQU_HUAXIAO@OLDHBDW) X)
                   WHERE RN = 1
                  ) E
      ...
      --物联网用户
      --EXECUTE IMMEDIATE 'TRUNCATE TABLE ZQ_DW_V_USER_HUAXIAO_IOT';
      DELETE FROM ZQ_DW_V_USER_HUAXIAO_IOT T
       WHERE T.ACCT_MONTH = V_ACCT_MONTH
         AND AREA_NO = C1.AREA_NO;
      COMMIT;
      INSERT INTO ZQ_DW_V_USER_HUAXIAO_IOT
      ...
      EXECUTE IMMEDIATE 'TRUNCATE TABLE ZQ_DW_V_USER_HUAXIAO_M_PLUS';
      INSERT INTO ZQ_DW_V_USER_HUAXIAO_M_PLUS
        SELECT *
          FROM (SELECT A.*,
                       ROW_NUMBER() OVER(PARTITION BY BUNDLE_ID ORDER BY(CASE
                         ...
                         ELSE
                          3
                       END)) RN
                  FROM ZQ_DW_V_USER_HUAXIAO_M A)
         WHERE RN = 1;
      COMMIT;

      ---------------------------- 向正式表写入数据 -------------------------------------
      EXECUTE IMMEDIATE 'ALTER TABLE ZQ_V_USER_HUAXIAO_INFO_M TRUNCATE SUBPARTITION PART' ||
                        SUBSTR(V_ACCT_MONTH, 1, 6) || '_SUBPART_' ||
                        C1.AREA_NO;

      INSERT INTO ZQ_V_USER_HUAXIAO_INFO_M
        SELECT /*+ORDERED*/
         V_ACCT_MONTH,
         A.AREA_NO,
         A.CITY_NO,
         ...
         CASE
           WHEN A.IS_KD_BUNDLE = '0' AND A.TELE_TYPE = '2' AND
                G.XIAOQU_NO IS NOT NULL AND G1.HUAXIAO_TYPE = '05' THEN
            '1' --对于融合解绑的用户，解绑后还是按照融合时固网装机地址

           WHEN A.IS_KD_BUNDLE = '0' AND A.TELE_TYPE = '2' AND
                D5.HUAXIAO_TYPE = '05'  AND G.XIAOQU_NO IS NULL THEN
            '1' --单C
           WHEN A.IS_KD_BUNDLE = '0' AND A.TELE_TYPE <> '2' AND
                E3.HUAXIAO_TYPE = '05' THEN
            '1' --单固
           WHEN  F.HUAXIAO_TYPE = '05' THEN
            '1' --融合
           ELSE
            '0'
         ...
         CASE
           ...
           WHEN  F.HUAXIAO_TYPE = '05' THEN
            F.HUAXIAO_NAME --融合 
         END HUAXIAO_NAME_05, --商客网格划小ID
         
         CASE
           WHEN D6.HUAXIAO_TYPE = '06' THEN
           D6.HUAXIAO_NAME  
         END HUAXIAO_NAME_06, --清单客户划小ID
         
         CASE
           WHEN D7.HUAXIAO_TYPE = '07' THEN
           D6.HUAXIAO_NAME
         END  HUAXIAO_NAME_07, --校园承包划小ID
         ...
         NVL(G3.IOT_FEE, 0)
          FROM (SELECT A.AREA_NO,
               ...
               (SELECT *
                 ...
                 (
                 SELECT *
                      FROM (SELECT X.*, ROW_NUMBER() OVER(PARTITION BY CHANNEL_NO ORDER BY 1) RN
                              FROM DIM.DIM_ZQ_CHANNEL_HUAXIAO@OLDHBDW X WHERE HUAXIAO_TYPE='05')
                     WHERE RN = 1 
                 )D5, -- 商客支局
                 (
                 SELECT *
                      FROM (SELECT X.*, ROW_NUMBER() OVER(PARTITION BY CHANNEL_NO ORDER BY 1) RN
                              FROM DIM.DIM_ZQ_CHANNEL_HUAXIAO@OLDHBDW X WHERE HUAXIAO_TYPE='06')
                     WHERE RN = 1 
                 )D6, -- 营维单元      
                 (
                 SELECT *
                      FROM (SELECT X.*, ROW_NUMBER() OVER(PARTITION BY CHANNEL_NO ORDER BY 1) RN
                              FROM DIM.DIM_GROUP_HX_GX@OLDHBDW X WHERE HUAXIAO_TYPE='07')
                     WHERE RN = 1 
                 )D7, -- 高校支局                               
               ...
               (SELECT *
                 ...
                 (
                 SELECT *
                      FROM (SELECT X.*, ROW_NUMBER() OVER(PARTITION BY XIAOQU_NO ORDER BY 1) RN
                              FROM DIM.DIM_ZQ_XIAOQU_HUAXIAO@OLDHBDW X WHERE HUAXIAO_TYPE='05')
                     WHERE RN = 1 
                 )E3, -- 商客支局   
               (SELECT * FROM ZQ_DW_V_USER_HUAXIAO_M_PLUS) F, --融合宽带用户

               (SELECT USER_NO, XIAOQU_NO, XIAOQU_NAME
                  FROM ZQ_V_USER_HUAXIAO_INFO_M G
               ...
               (SELECT T.USER_NO, T.IOT_FEE
                  FROM ZQ_DW_V_USER_HUAXIAO_IOT T
                 WHERE T.ACCT_MONTH = V_ACCT_MONTH
         ...
         WHERE A.USER_NO = B.USER_NO(+)
           ...
           AND A.CHANNEL_NO = D6.CHANNEL_NO(+)
