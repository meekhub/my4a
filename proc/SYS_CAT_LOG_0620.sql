...
SELECT SUBSTR(T.CREATE_DATE, 1, 8) AS DAY_ID,
       CASE WHEN UPPER(T.USER_ID) LIKE 'LF%' THEN '183' ELSE NVL(Y.ATTR_VALUE, '018') END AS AREA_NO,
       '1' SYS_KIND,
...
SELECT TO_CHAR(T.CREATE_DATE, 'YYYYMMDD'),
       ...
       T.USER_ID,
       X.OA_UID,
       nvl(X.USER_NAME,z.user_name),
       X.MEMO,
       NVL(T.MANAGER_TELEPHONE,nvl(X.MOBILE,z.mobile)),
       T.HUAXIAO_TYPE,
       T.HUAXIAO_TYPE_NAME,
       T.HUAXIAO_NO,
       T.HUAXIAO_NAME,
