...
BEGIN
    ...
    SELECT ACCT_MONTH,
     ...
     WHERE ACCT_MONTH = TO_CHAR(SYSDATE - 1, 'YYYYMM')
       ...
       AND USER_DINNER IN ('1394904',
        ...
        AND TO_CHAR(INNET_DATE, 'YYYYMMDD') >= '20171110' --VIVO X9sPlus：7月8日-7月14日
        AND TO_CHAR(INNET_DATE, 'YYYYMMDD') <= '20171116';
  ...
  INSERT INTO TMP170608_ZQP_USER_FIRST_D
    SELECT DEVICE_NO, TERMINAL_CODE, TERMINAL_MODEL, REG_DATE
     ...
     WHERE RN = 1
       AND TERMINAL_MODEL = 'OB-OPPO R11s' --OB-OPPO R11s：7月20日-7月26日 
       AND TO_CHAR(REG_DATE, 'YYYYMMDD') >= '20171110' --VIVO X9sPlus：7月8日-7月14日
       AND TO_CHAR(REG_DATE, 'YYYYMMDD') <= '20171116';--用户首次注册时间为活动区间
     COMMIT;
