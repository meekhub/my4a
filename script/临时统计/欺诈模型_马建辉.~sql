CREATE OR REPLACE PROCEDURE P_CHECK_USER_ALARM_D(V_DATE    VARCHAR2,
                                                     V_RETCODE OUT VARCHAR2,
                                                     V_RETINFO OUT VARCHAR2) IS
  /*****************************************************************
  *名称 --%NAME:P_CHECK_USER_ALARM_D
  *执行周期 --%PERIOD: 日
...
BEGIN
  V_PKG        := '每日欺诈用户预警';
  V_PROCNAME   := 'P_CHECK_USER_ALARM_D';
  ...
  IF V_COUNT >= 3 THEN
    ...
    ------------------1、提取每日漫游至龙岩、漳州话单---------------------
    --EXECUTE IMMEDIATE 'TRUNCATE TABLE dm_cheat_mid_01';
    ...
    ------------------5、输出风险用户---------------------
    DELETE FROM DM_CHEAT_RISK_USER WHERE DAY_ID = V_DATE;
    COMMIT;
