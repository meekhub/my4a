...
BEGIN
    ...
    SELECT ACCT_MONTH,
     ...
     WHERE ACCT_MONTH = TO_CHAR(SYSDATE - 1, 'YYYYMM')
       ...
       AND USER_DINNER IN ('1394904',
        ...
        AND TO_CHAR(INNET_DATE, 'YYYYMMDD') >= '20171106' --VIVO X9sPlus：7月8日-7月14日
        AND TO_CHAR(INNET_DATE, 'YYYYMMDD') <= '20171112';
  ...
  INSERT INTO TMP170608_ZQP_USER_FIRST_D
    SELECT DEVICE_NO, TERMINAL_CODE, TERMINAL_MODEL, REG_DATE
     ...
     WHERE RN = 1
       AND TERMINAL_MODEL = 'GIO-GIONEE M7' --GIO-GIONEE M7：7月20日-7月26日 
       AND TO_CHAR(REG_DATE, 'YYYYMMDD') >= '20171106' --VIVO X9sPlus：7月8日-7月14日
       AND TO_CHAR(REG_DATE, 'YYYYMMDD') <= '20171112';--用户首次注册时间为活动区间
     COMMIT;
...
terminal_model, 
reg_date
   FROM TMP170720_ZQP_BKZD_BASE_AIM_U WHERE OPER_DAY  = TO_CHAR(SYSDATE - 1, 'YYYYMMDD') ;
