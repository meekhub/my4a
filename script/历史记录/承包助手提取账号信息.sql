...
---提取所有账号信息
create table tmp_majh_cbzs_zjk as 
select distinct h.description ,u.login_id,u.mobile,u.user_name,decode(u.state,'1','是','否')state
  from  
       mobile_cbzs.e_user                     u,
       mobile_cbzs.e_user_attribute           f,
       mobile_cbzs.c_dmcode_city              H
 where u.login_id = f.user_id(+)
   AND F.ATTR_VALUE = H.CITY_ID(+)
   and f.attr_code = 'CITY_NO'
   and h.area_description='衡水' 
...
user_name,      
   case when r.user_Id is not null then '已有角色cbzs_ceo' else '没有角色cbzs_ceo' end isflag
from tmp_majh_cbzs_zjk t,(select user_id from mobile_cbzs.e_user_role r where r.role_code = 'cbzs_ceo')r
where  t.login_id =r.user_id(+)    
