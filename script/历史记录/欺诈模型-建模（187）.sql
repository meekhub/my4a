--区分后付费、预付费
truncate table tmp_majh_qizha_pre_base_187
insert into tmp_majh_qizha_pre_base_187
select device_number,decode(is_ocs,'是','1','0')
from DW_V_USER_BASE_INFO_USER where acct_month='201706' and is_onnet='是'
and tele_type='2' 
and area_no='187'

select is_ocs,count(*) from tmp_majh_qizha_pre_base_187 group by is_ocs

truncate table TMP_MAJH_QIZHA_02_pre_187

--沉淀基础指标(后付费)
INSERT INTO TMP_MAJH_QIZHA_02_pre_187
  SELECT  
         A.DEVICE_NUMBER,
         nvl(SUM(CASE
    ...
    FROM (SELECT * FROM tmp_majh_qizha_pre_base_187 WHERE IS_OCS = '0') A
    LEFT JOIN (SELECT *
...
INSERT INTO TMP_MAJH_QIZHA_02_pre_187
  SELECT  
         ...
         nvl(SUM(CASE
               ...
               ELSE
                0
             END),0) LONG_DURATION --长途通话时长
    FROM (SELECT * FROM tmp_majh_qizha_pre_base_187 WHERE IS_OCS = '1') A
    LEFT JOIN (SELECT *
 ...
 --提取宽表信息
 truncate table TMP_MAJH_QIZHA_04_pre_187
insert into TMP_MAJH_QIZHA_04_pre_187
SELECT 
	  x.area_no,
	  x.area_no_desc,
	  x.city_no,
  ...
  (select * from (SELECT B.*,
          ...
          FROM (SELECT b.area_no,
                       ...
                       b.terminal_code,
                       b.user_dinner
                  FROM DW_V_USER_BASE_INFO_USER B, tmp_majh_qizha_pre_base_187 C
...
--计算用户在某基站下通话最多
insert into tmp_majh_qizha_cellmax_pre_187
...
--样本集汇总
truncate table tmp_majh_qizha_hz_01_pre_187
insert into tmp_majh_qizha_hz_01_pre_187 
select 
'201706',
area_no,
area_no_desc,
...
nvl(a.is_cert_1_n,'0') is_cert_1_n --是否一证三卡用户
 from TMP_MAJH_QIZHA_04_pre_187 a --基础信息表
left join
TMP_MAJH_QIZHA_02_pre_187 b --近4个月呼叫信息
on a.device_number=b.device_number 
left join
...
left join
tmp_majh_qizha_cellmax_pre_187 f --通话最多的基站
...
select count(*) from tmp_majh_qizha_hz_01_pre_187 where call_duration>0 and call_cdr>0


select count(*) from tmp_majh_qizha_hz_01_pre_187 a,  tmp_majh_qizha_hz_01 b where a.device_number=b.device_number

select * from tmp_majh_qizha_hz_01 where device_number='17703311673'

select ceil(cell_rate),count(*) from tmp_majh_qizha_hz_01_pre_187 group by ceil(cell_rate)

select * from tmp_majh_qizha_hz_01_pre_187 where cell_rate=100 and call_cdr>800
...
(
--规则1
select a.device_number from tmp_majh_qizha_hz_01_pre_187 a 
...
on a.device_number=b.device_number
join 
tmp_majh_qizha_hz_01_pre_187 c
