truncate table tmp_majh_qizha_pre
--预测用户
insert into tmp_majh_qizha_pre
select device_number from DW_V_USER_BASE_INFO_USER where acct_month='201705' and is_onnet='是'
and area_no='188'
and tele_type='2' 

--区分后付费、预付费
truncate table tmp_majh_qizha_zc_base
insert into tmp_majh_qizha_zc_base
select 
a.device_number,decode(b.is_ocs,'是','1','0') is_ocs
from tmp_majh_qizha_zc a
left join
(
select device_number,is_ocs from DW_V_USER_BASE_INFO_USER where acct_month='201705' and is_onnet='是'
and tele_type='2' 
)b
on a.device_number=b.device_number

select is_ocs,count(*) from tmp_majh_qizha_zc_base group by is_ocs
...
INSERT INTO TMP_MAJH_QIZHA_02_ZC
  SELECT '201705' ACCT_MONTH,
    ...
    FROM (SELECT * FROM tmp_majh_qizha_zc_base WHERE IS_OCS = '0') A
    LEFT JOIN (SELECT *
  ...
  SELECT '201705' ACCT_MONTH,
         ...
         nvl(SUM(CASE
               ...
               ELSE
                0
             END),0) LONG_DURATION --长途通话时长
    FROM (SELECT * FROM tmp_majh_qizha_zc_base WHERE IS_OCS = '1') A
    LEFT JOIN (SELECT *
...
SELECT x.DEVICE_NUMBER,
  ...
  (select * from (SELECT B.*,
          ...
          FROM (SELECT   
                       ...
                       b.terminal_code,
                       b.user_dinner
                  FROM DW_V_USER_BASE_INFO_USER B, tmp_majh_qizha_zc_base C
...
--欺诈用户清单
select device_number from tmp_majh_qizha_zc_base
...
nvl(a.is_cert_1_n,'0') is_cert_1_n --是否一证三卡用户
 from TMP_MAJH_QIZHA_04_ZC a --基础信息表
left join
TMP_MAJH_QIZHA_03_ZC b --近4个月呼叫信息
on a.device_number=b.device_number 
left join
...
left join
tmp_majh_qizha_cellmax_zc f --通话最多的基站
...
select a.*,'0' is_risk_user  from tmp_majh_qizha_hz_01_zc a where call_duration>0 and call_cdr>0
union
select b.*,'1' is_risk_user from tmp_majh_qizha_hz_01 b where call_duration>0 and call_cdr>0
