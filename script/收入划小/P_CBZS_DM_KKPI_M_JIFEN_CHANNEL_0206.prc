CREATE OR REPLACE PROCEDURE P_CBZS_DM_KKPI_M_JIFEN_CHANNEL(V_ACCT_MONTH VARCHAR2,
  ...
  /*-----------------------------------------------------------------------
  过 程 名：P_CBZS_DM_KKPI_M_JIFEN_CHANNEL
  生成日期：2017年11月28
  编 写 人：liangzhitao
  周  期  ：月
  目 标 表：CBZS_DM_KKPI_M_JIFEN_CHANNEL
  过程说明：(新划小)积分月报
  -----------------------------------------------------------------------*/
  V_PKG        VARCHAR2(40);
...
/*  if substr(V_LAST_MONTH,1,4) <  substr(V_ACCT_MONTH,1,4) then
  ...
  IF V_COUNT1 >= 1 THEN
    ...
    ------插入一级汇总数据
    delete from MOBILE_CBZS.CBZS_DM_KKPI_M_JIFEN
    where acct_month=v_acct_month;
    commit;
    INSERT INTO MOBILE_CBZS.CBZS_DM_KKPI_M_JIFEN
      SELECT ACCT_MONTH,
             ...
             SUM(YEAR_VALUE)
        FROM MOBILE_CBZS.CBZS_DM_KKPI_M_JIFEN_CHANNEL
       WHERE ACCT_MONTH = V_ACCT_MONTH
       GROUP BY ACCT_MONTH,
                AREA_NO,
                CITY_NO,
    ...
    ------插入二级汇总数据
    delete from MOBILE_CBZS.CBZS_DM_KKPI_M_JIFEN_ADMIN
    where acct_month=v_acct_month;
    commit;
