CREATE OR REPLACE PROCEDURE P_TMP_CNTRT_MGMT_SUM_M(V_MONTH   VARCHAR2,
                                                   V_RETCODE OUT VARCHAR2,
                                                   V_RETINFO OUT VARCHAR2) IS
  /*-----------------------------------------------------------------------
  过 程 名：P_TMP_CNTRT_MGMT_SUM_M
  生成日期：2014年7月28
  编 写 人：MAJIANHUI
  周  期  ：月
  目 标 表：BWT_CNTRT_MGMT_SUM_M
  过程说明：承包经营量收、成本、预算汇总指标（月）
  -----------------------------------------------------------------------*/
  V_PKG      VARCHAR2(40);
...
BEGIN
  V_PKG      := 'P_BWT_CNTRT';
  V_PROCNAME := 'P_TMP_CNTRT_MGMT_SUM_M';
  ...
  SELECT COUNT(1)
   ...
   WHERE ACCT_MONTH = V_MONTH
     AND RESULT = 'SUCCESS'
     AND PROCNAME IN ('P_TMP_CNTRT_MGMT_CHNL_REL_M');
  COMMIT;
