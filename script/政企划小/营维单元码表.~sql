...
--整理
select a.* from tmp_majh_yw_hx_2 a,dim.dim_area_no b,dim.dim_city_no c
where a.area_desc=b.area_desc(+) 
and a.city_desc=c.city_desc(+)
and c.city_desc is null; 

create table dim.dim_hx_yw as
...
b.area_no,c.city_no,d.city_no_new,a.huaxiao_name,a.huaxiao_type_name,a.huaxiao_level
 from tmp_majh_yw_hx_2 a,dim.dim_area_no b,dim.dim_city_no c,
...
select * from tmp_majh_yw_hx_3 a, dim_hx_yw b,dim.dim_area_no c,dim.dim_city_no d
...
--比对不上的
select * from tmp_majh_yw_hx_3 t,dim_hx_yw x
...
(select t.huaxiao_name from tmp_majh_yw_hx_4 t,dim_hx_yw x
where t.huaxiao_name=x.huaxiao_name(+)
and x.huaxiao_name is null) for update
 

--创建关系表
create table dim_hx_yw_channel as
...
b.huaxiao_type_name_big
 from 
tmp_majh_yw_hx_4 a,dim_hx_yw b,dim.dim_area_no c,dim.dim_city_no d
where a.area_desc=c.area_desc
and a.city_no=d.city_desc
and a.huaxiao_name=b.huaxiao_name
and c.area_no=d.area_no;

update dim_hx_yw_channel set channel_no=replace(channel_no,' ','')
...
a.area_no,b.area_desc,a.city_no,c.city_desc,a.huaxiao_no,a.huaxiao_name,a.huaxiao_type,a.huaxiao_type_name
 from dim.dim_hx_yw a,dim.dim_area_no b,dim.dim_city_no c
...
select 
huaxiao_no,
substr(huaxiao_no,1,5)||'001'||substr(huaxiao_no,6,7)
 from dim.dim_hx_yw t where t.huaxiao_no like '%0000000'; 
 
 
 
update dim.dim_hx_yw t set huaxiao_no=substr(huaxiao_no,1,5)||'001'||substr(huaxiao_no,6,7)
where t.huaxiao_no like '%0000000';
