...
--评估效果汇总
 SELECT  A.ACCT_MONTH,
         B.AREA_DESC, 
         sum(A.MINUS_CNT_LJ)MINUS_CNT_LJ,
         sum(A.DEV_CNT_LJ)DEV_CNT_LJ,
         sum(A.MINUS_FEE_LJ)MINUS_FEE_LJ
FROM 
    ( SELECT *
    FROM ALLDM.DM_BOX_EFFECT_EVAL T
    WHERE 1 = 1 {AND T.ACCT_MONTH = #ACCT_MONTH#} {AND T.AREA_NO = #AREA_NO#} {AND CITY_NO = #CITY_NO#} {AND T.PROJECT_TYPE = #PROJECT_TYPE#}) A, 
    ( SELECT AREA_NO FACT_AREA_ID,
         DESCRIPTION AREA_DESC,
         IDX_NO SORT_ID,
         AREA_NO AREA_ID
    FROM NEW_ALLDMCODE.DMCODE_AREA) B, 
    ( SELECT CITY_NO FACT_AREA_ID,
