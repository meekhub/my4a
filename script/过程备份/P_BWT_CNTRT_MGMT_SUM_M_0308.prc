...
BEGIN
  ...
  IF V_COUNT1 >= 5 AND V_COUNT2 >= 1 THEN
    ...
    --移动出账用户到达数（当期）
      SELECT V_MONTH,
        ...
        FROM (SELECT CHANNEL_NO, SUM(ACCT_USER) ACCT_USER
             ...
             (SELECT CHANNEL_NO, HUAXIAO_NO AS CNTRT_MGMT_UNIT_CD
                FROM DIM.DIM_CHANNEL_HUAXIAO) C
       WHERE A.CHANNEL_NO = C.CHANNEL_NO
       GROUP BY C.CNTRT_MGMT_UNIT_CD
      /*UNION ALL
      --移动出账用户本年净增
      SELECT V_MONTH,
             '813',
   ...
   --清除不在基础表中数据
   delete from BWT_CNTRT_MGMT_SUM_M a where acct_month=V_MONTH
   and not exists 
   (select 1 from BWT_CNTRT_MGMT_UNIT_M b where acct_month=V_MONTH
   and a.cntrt_mgmt_unit_cd=b.cntrt_mgmt_unit_cd);
   commit;
