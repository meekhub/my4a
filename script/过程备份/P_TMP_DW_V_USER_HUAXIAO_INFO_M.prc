CREATE OR REPLACE PROCEDURE P_TMP_DW_V_USER_HUAXIAO_INFO_M(V_ACCT_MONTH VARCHAR2,
...
/*      --取上月固网融合用户
      EXECUTE IMMEDIATE 'TRUNCATE TABLE MID_DW_V_USER_HUAXIAO_LM';
      INSERT INTO MID_DW_V_USER_HUAXIAO_LM
        SELECT A.USER_NO,
               A.DEVICE_NUMBER,
               A.TELE_TYPE,
               A.BUNDLE_ID,
      ...
      INSERT INTO MID_DW_V_USER_HUAXIAO_IOT
        SELECT A.AREA_NO, A.USER_NO, SUM(A.REAL_PRICE_FEE2)
          FROM (SELECT *
      ...
      INSERT INTO DW_V_USER_HUAXIAO_INFO_M
        SELECT /*+ORDERED*/
         ...
         CASE
           ...
           WHEN A.TELE_TYPE_NAME IN ('移动', '宽带', '电视', '固话') AND
                A.IS_KD_BUNDLE = '0' AND A.TELE_TYPE = '2' AND
                G.HUAXIAO_NO_03 IS NOT NULL AND G1.HUAXIAO_TYPE = '03' THEN
         ...
         CASE
           ...
           WHEN F.HUAXIAO_TYPE = '04' THEN
            '1'
           WHEN A.IS_KD_BUNDLE = '0' AND A.TELE_TYPE = '2' AND
                G.HUAXIAO_NO_04 IS NOT NULL AND G2.HUAXIAO_TYPE = '04' THEN
            '1'
           ELSE
            '0'
         ...
         NVL(G3.IOT_FEE, 0)
          FROM (SELECT A.AREA_NO,
               ...
               (SELECT USER_NO,HUAXIAO_NO_03,HUAXIAO_NO_04 FROM DW_V_USER_HUAXIAO_INFO_M G WHERE ACCT_MONTH=V_LAST_MONTH) G, --上月沉淀的基础表
               (SELECT *
                  FROM DIM.DIM_XIAOQU_HUAXIAO
               ...
               (SELECT T.USER_NO, T.IOT_FEE FROM MID_DW_V_USER_HUAXIAO_IOT T) G3 --物联网用户
         ...
         WHERE A.USER_NO = B.USER_NO(+)
           ...
           AND G.HUAXIAO_NO_03 = G1.XIAOQU_NO(+)
           AND G.HUAXIAO_NO_04 = G2.XIAOQU_NO(+)
